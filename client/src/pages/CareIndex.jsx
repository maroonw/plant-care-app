...
export default function CareIndex() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const r = await api.get('/content/list', { params: { type: 'care' } });
        setItems(r.data || []);
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold text-green-900 mb-6">Care Guides</h1>
      {items.length === 0 ? (
        <div className="text-gray-600">No guides yet.</div>
      ) : (
          ...
          {items.map(p => {
            ...
            return (
              <li key={slug} className="bg-white rounded shadow p-4">
                <a
                  href={`/care/${slug}`} // server-rendered page
