...
export default function EditCareScheduleModal({ userPlant, onSave, onClose }) {
  ...
  const rec = { wateringFrequencyDays: 7, fertilizingFrequencyDays: 30, repotIntervalMonths: 18, rotateIntervalDays: 14 };
    ...
    <ModalBase onClose={onClose}>
      ...
      <form onSubmit={submit} className="space-y-4">
        <label className="flex items-center gap-2">
          <input type="checkbox" className="accent-green-600" checked={isCustom} onChange={(e) => setIsCustom(e.target.checked)} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" min="1" disabled={!isCustom} value={watering} onChange={(e) => setWatering(e.target.value)} className="w-full border rounded px-3 py-2" />
          </div>
          <div>
            ...
            <input type="number" min="1" disabled={!isCustom} value={fertilizing} onChange={(e) => setFertilizing(e.target.value)} className="w-full border rounded px-3 py-2" />
          </div>
          <div>
            ...
            <input type="number" min="1" disabled={!isCustom} value={repot} onChange={(e) => setRepot(e.target.value)} className="w-full border rounded px-3 py-2" />
          </div>
          <div>
            ...
            <input type="number" min="1" disabled={!isCustom} value={rotate} onChange={(e) => setRotate(e.target.value)} className="w-full border rounded px-3 py-2" />
          </div>
        </div>
